...
<body>
    ...
    <div class="container">
        ...
        <div class="main">
            ...
            <div class="role-list">
                <h3>Bank B Information</h3>
                <?php
                    $select_query = "SELECT * FROM BankB";
                    $result = mysqli_query($conn, $select_query);
                    $userId = $_SESSION['user_id'];


                    $hasPostAccess = checkUserAccessPost($conn, $userId, 'Bank B');
                    if ($hasPostAccess) : ?>
                        <a href="../bankB_action/add_BankB.php" class="add-user-button">Add Product</a>
                <?php endif; ?>
                ...
                <table>
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        $hasUpdateAccess = checkUserAccessUpdate($conn, $userId, 'Bank B'); // Replace $userId with actual user ID
                        $hasDeleteAccess = checkUserAccessDelete($conn, $userId, 'Bank B');
                        ...
                        if ($hasUpdateAccess) {
                            echo '<a href="../bankB_action/edit_bankB.php?product_id=' . $product_id . '" class="edit-button">Edit</a>';
                        }
                        echo '</td>';
                        echo '<td class="actions-cell">';
    ...
    <script>
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', () => {
                ...
                xhr.open('GET', `delete_productB.php?delete_product=${productId}`, true);
